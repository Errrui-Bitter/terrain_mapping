...
void rsHandler_XYZI(sensor_msgs::PointCloud2 pc_msg)
{
    ...
    for (int point_id = 0; point_id < pc->points.size(); ++point_id)
    {
        ...
        // wxr fix 6t lidar
        float range = sqrt(new_point.x * new_point.x + new_point.y * new_point.y + new_point.z * new_point.z);
        if (range > 60.0 || range < 2)
        {
...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        {
            // subRobosensePC = nh.subscribe("/lidar_cloud_calibrated", 1, rsHandler_XYZI);
            // subRobosensePC = nh.subscribe("/rslidar_points", 1, rsHandler_XYZI);
            subRobosensePC = nh.subscribe("/rslidar_points_80", 1, rsHandler_XYZI);
        }
        else if (std::strcmp("XYZIRT", argv[1]) == 0)
        {
            // subRobosensePC = nh.subscribe("/lidar_cloud_calibrated", 1, rsHandler_XYZIRT);
            // subRobosensePC = nh.subscribe("/rslidar_points", 1, rsHandler_XYZIRT);
            subRobosensePC = nh.subscribe("/rslidar_points_80", 1, rsHandler_XYZIRT);
        }
        else
        {
            ROS_ERROR(argv[1]);
    ...
    // ROS_INFO("Listening to /lidar_cloud_calibrated ......");
    // ROS_INFO("Listening to /rslidar_points ......");
    ROS_INFO("Listening to /rslidar_points_80 ......");
    ros::spin();
    return 0;
}
